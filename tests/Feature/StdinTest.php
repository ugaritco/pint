...
it('renames the class to the stdin-filename across common Ugarit paths', function (string $path, string $class) {
    $input = <<<'PHP'
    <?php
...
it('leaves Ugarit files whose name is not a class name untouched', function (string $path, string $input, string $expected) {
    $result = Process::input($input)
        ->path(base_path('tests/Fixtures/psr-autoloading'))
        ->run('php '.base_path('pint').' --stdin-filename='.escapeshellarg($path))
        ->throw();
